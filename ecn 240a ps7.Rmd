...
# generate contemporary futures price (for demand regressions)
RSdata$ln_cfp <- log(RSdata$fut_price / RSdata$cpi)
...
# demand model
demandOLS <- lm(data = RSdata, ln_qd ~ ln_cfp + trendsp1+ trendsp2+ trendsp3)
...
demandIV.firststage <- lm(data = RSdata, formula = ln_cfp ~ ln_w + trendsp1 + trendsp2 + trendsp3)

summary(demandIV.firststage)

demandIV <- ivreg(data = RSdata, formula = ln_qd ~ ln_cfp + trendsp1 + trendsp2 + trendsp3 | trendsp1 + trendsp2 + trendsp3 + ln_w)
...
``` {r tables}
kable(compare.supplyIV, digits = 3, caption = "IV regressions")
```
